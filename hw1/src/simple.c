...
static int simple_init(void)
{
    ...
    /////////////////////
    unsigned long jiff_now;
    jiff_now = jiffies;
    ////<在此加入程式碼>////
    /////////////////////
    printk(KERN_INFO "init_jiffies: %lu\n", jiff_now); // 印出載入時的jiffies，使用％lu來表示，格式為"init_jiffies: XXXXXXXXXXX"，記得換行
...
static void simple_exit(void) {
    ...
    //////////////////////
    ////<在此加入程式碼>////
    unsigned long end_jiff = jiffies;
    /////////////////////>
    printk(KERN_INFO "exit_jiffies: %lu\n", end_jiff);//  印出移除時的jiffies，使用％lu來表示，格式為"exit_jiffies: XXXXXXXXXXX"，記得換行
